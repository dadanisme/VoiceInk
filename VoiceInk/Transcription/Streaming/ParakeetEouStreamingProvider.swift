...
/// Wraps FluidAudio's StreamingEouAsrManager as a VoiceInk StreamingTranscriptionProvider.
...
final class ParakeetEouStreamingProvider: StreamingTranscriptionProvider {

    private let logger = Logger(subsystem: "com.prakashjoshipax.voiceink", category: "ParakeetEouStreaming")
    ...
    /// main actor when it reads the chunk size) can pass the value in without
    /// actor-isolation issues.
    init(chunkSize: StreamingChunkSize) {
        self.chunkSizeMs = chunkSize.durationMs
    ...
    func connect(model: any TranscriptionModel, language: String?) async throws {
        ...
        logger.notice("ParakeetEou streaming started (\(self.chunkSizeMs)ms chunks)")
    }
    ...
    func sendAudioChunk(_ data: Data) async throws {
        ...
        do {
            ...
            logger.error("ParakeetEou process failed: \(error.localizedDescription, privacy: .public)")
    ...
    func disconnect() async {
        ...
        eventsContinuation?.finish()
        logger.notice("ParakeetEou streaming disconnected")
    }
